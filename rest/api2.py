...
@app.route('/api/update_data', methods=['PATCH'])
def update_data():
    # Obtener los datos del cuerpo de la solicitud
    data = request.get_json()
    id_area = data.get('id_area')
    ...
    # Validar los datos
    if not (id_area and id_sensor and humedad is not None and consumo is not None and record_id):
        return jsonify({'error': 'Faltan parámetros requeridos'}), 400
    ...
    # Hacer la llamada PATCH a la API externa
    try:
        response = requests.patch(f'https://smartriego.pockethost.io/api/collections/estadisticas/records/{record_id}', json=payload)
        response.raise_for_status()  # Lanza un error si la solicitud falla
...
# curl -X PATCH http://127.0.0.1:5000/api/update_data -H "Content-Type: application/json" -d '{"id_area": "area1", "id_sensor": "sensor1", "humedad": 80.0, "consumo": 150.0, "id": "1705qct21w7kled"}'